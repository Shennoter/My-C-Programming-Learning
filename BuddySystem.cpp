...
private:
    ...
    int startAddr = 0;
    PCB* pcbHead = new PCB;
    Block* blockList[LISTNUM];
    Block* mainHead = new Block;

    void traverseList(int); // 遍历链表
    void memCollect(); // 内存回收
    Block* splitBlock(Block*, int); // 分割区块并加入下一条链表。参数：1. 需要切割的区块 2. 目标大小
    Block* findBlock(int, int, bool); // 寻找可用的分区。参数：1.目标分区大小 2.分区大小（递归传参用） 3.是否切割
    void updateMain(int, Block*); // 更新主链表（切割）。参数：1.要切割的分区的id 2.需要复制的区块
    void updateMain(Block*); // 更新主链表（不切割）。参数：需要复制的区块
...
void BuddySystem::memCollect() {
    ...
    while (mainHead->next->next) {
        ...
        int temp1 = pow(2, fNode->sizeK + 1);
        if (fNode->startAddr % temp1 == 0) {
